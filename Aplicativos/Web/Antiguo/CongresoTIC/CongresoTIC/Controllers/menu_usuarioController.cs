    ...
    public class menu_usuarioController : ApiController
    {
        ...
        public menu_usuario[] data()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    row = dt.Rows[i];
                    menu_usuarios[i] = new menu_usuario(Convert.ToInt32(row["fk_idsubitem"].ToString()), Convert.ToInt32(row["fk_idrol"].ToString()), row["estado"].ToString());
                }
            }
        ...
        {
            var obj = data().FirstOrDefault((o) => o.fk_idsubitem == id);
            if (obj != null)
            {
                return Ok(obj);
            }
